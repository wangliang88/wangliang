...
def random_scale_image(image:np.ndarray,interpolation=cv2.INTER_CUBIC):
...
def random_scale_pair_image(image:np.ndarray, label:np.ndarray):
    f_scale = 0.5 + random.randint(0, 10) / 10.0
    image = cv2.resize(image, None, fx=f_scale, fy=f_scale, interpolation=cv2.INTER_CUBIC)
    label = cv2.resize(label, None, fx=f_scale, fy=f_scale, interpolation=cv2.INTER_CUBIC)
    return image, label
...
def square_crop(img:np.ndarray, label:np.ndarray, need_size: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    label = cv2.resize(label, dsize=(need_size, need_size), interpolation=cv2.INTER_CUBIC)
    return img, label
