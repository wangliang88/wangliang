...
import torch
import os
from helper.ematrainer import Trainer
...
def train(config:Configuration) -> None:
    ...
    if partial_size < dataset_size:
        ...
        semi_wrapper = None
    mse_loss_function = MSE_LOSS(ignore_value=sal_dataset.ignore_value).to(device)
    ...
    G_optim_obj_1 = Optimizer(G_optim_create_func, config.MAX_ITER,
                          step_time_interval=config.STEP_INTERVAL,
                            lr_schuduer=G_adjust_lr)
    G_optim_obj_2 = Optimizer(G_optim_create_func, config.MAX_ITER,
                          step_time_interval=config.STEP_INTERVAL,
                            lr_schuduer=G_adjust_lr)
    ...
    trainer = Trainer(
        ...
        config.MAX_ITER,
        ignore_value = sal_dataset.ignore_value,
        generator_optim_create_func_1 = G_optim_obj_1,
        generator_optim_create_func_2 = G_optim_obj_2,
        discriminator_optim_create_func = D_optim_obj,
        generate_1_lr = config.G_LEARNING_RATE,
        generate_2_lr = config.G_LEARNING_RATE,
